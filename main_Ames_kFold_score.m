%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%   Perform the biomarker selection for the Ames genotoxicity prediction
%   This study is cinducted using the yeast proteomic assay
%   Developed by: Sheikh M. Rahman
%   Date: March, 2018
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

load ('data/peli_ames.mat'); % load carcinogenicity data
...
print (h3, '-dpdf', '-r300', 'results/biomarkerScore_ames_10fold.pdf');
...
classModel_top5 = fitcsvm(peli_ames(:,id), classLabel, 'kfold', 10, 'kernelfunction', 'linear');
...
classModel_all = fitcsvm(peli_ames(:,id), classLabel, 'kfold', 10, 'kernelfunction', 'linear');
...
writetable(table(AUC_train', accuracy_train', sensitivity_train', specificity_train',...
        ...
        'results/ames_accuracy_top5_topall.csv', 'WriteRowNames',1);

% save the models as .mat file
save ('results/predictionModels_ames.mat','classModel_top5','classModel_all','classLabel');

%% Plot the heatmap of protein PELI with the biomarker sorted based on MRMR-TCQ score (Figure 4)
% prepare data
peliGene = peli_ames;
allChemName = allChemName_ames;
...
classLabel_ord = [classLabel(id_positive); classLabel(id_negative)];
idSorted_ames = featureIdsorted.mrmrTCQ;
...
set(fh, 'PaperUnits','inches', 'PaperSize',[7 8], 'PaperPosition',[0 0 7 8],...
    'Units','inches', 'Position',[3 3 7 8]);
axes('position',[0.34 0.1 0.645 0.88]);
imagesc(peliGene_ord(:,idSorted_ames));
d=jet(36); colormap(d([4,20:36],:));
set(gca,'xtick',1:1:numGene, 'xticklabel',geneName_N(idSorted_ames),'xticklabelrotation',90,...
    'ytick',0.5:6:numSample+0.5, 'yticklabel','', 'tickdir','out', 'fontname','arial','fontsize', 10);
...
xlabel('Proteins', 'fontname', 'arial', 'fontsize', 15);
ha = axes('position',[0.012 0.365 0.05 0.52]);
